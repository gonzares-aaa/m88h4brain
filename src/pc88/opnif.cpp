...
// ---------------------------------------------------------------------------
//	$Id: opnif.cpp,v 1.18 1999/11/26 10:13:48 cisc Exp $
...
//	初期化
//
bool OPNIF::Init(IOBus* b, int intrport, int io, int i, Scheduler* s)
{
	bus = b;
	scheduler = s;
	portio = io;
...
bool IFCALL OPNIF::SetRate(uint rate)
{
	...
	opn.SetRate(3993600, rate, fmmixmode);
	currentrate = rate;
	return true;
}
