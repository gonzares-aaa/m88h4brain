...
//	uPD765A
// ---------------------------------------------------------------------------
//	$Id: fdc.cpp,v 1.17 1999/11/26 10:13:46 cisc Exp $
...
//	初期化
//
bool FDC::Init(DiskManager* dm, Scheduler* s, IOBus* b, int ip)
{
	diskmgr = dm;
	scheduler = s;
	bus = b;
...
// ---------------------------------------------------------------------------
//	10	Invalid,		ScanEqual,		Invalid,		Invalid,
//	18	Invalid,		ScanLowOrEqual,	Invalid,		Invalid,
//	1c	Invalid,		ScanHighOrEqual,Invalid,		Invalid,
//
const FDC::CommandFunc FDC::CommandTable[32] =
{
	...
	CmdInvalid,				CmdSeek,
	CmdInvalid,				CmdInvalid,			// 10
	CmdInvalid,				CmdInvalid,			
	CmdInvalid,				CmdInvalid,			// 14
	CmdInvalid,				CmdInvalid,
	CmdInvalid,				CmdInvalid,			// 18
	CmdInvalid,				CmdInvalid,
	CmdInvalid,				CmdInvalid,			// 1c
	CmdInvalid,				CmdInvalid,
};
...
void FDC::ReadData(bool deleted)
{
	LOG4("\tRead %.2x %.2x %.2x %.2x\n", idr.c, idr.h, idr.r, idr.n);
	if (showstatus)
		statusdisplay.Show(85, 0, "Read (%d) %.2x %.2x %.2x %.2x", hdu & 3, idr.c, idr.h, idr.r, idr.n);
...
void FDC::Seek(uint dr, uint cy)
{
	...
	else
	{
		...
		seektime = 10 * Abs(seekcount) + 10;
		scheduler->AddEvent
			(10*Abs(seekcount)+10, this, STATIC_CAST(TimeFunc, &FDC::SeekEvent), dr);
		seekstate |= 1 << dr;
	}
}
...
bool IFCALL FDC::LoadStatus(const uint8* s)
{
	...
	if (st->t_phase != idlephase)
		timerhandle = scheduler->AddEvent(100, this, STATIC_CAST(TimeFunc, &FDC::PhaseTimer), st->t_phase);
	...
	for (int d=0; d<num_drives; d++)
	{
		...
		if (seekstate & (1 << d))
			scheduler->AddEvent(100, this, STATIC_CAST(TimeFunc, &FDC::SeekEvent), d);
		statusdisplay.FDAccess(d, drive[d].hd != 0, false);
	}
