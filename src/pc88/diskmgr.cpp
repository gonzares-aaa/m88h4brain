...
{
	// 既に持っているファイルかどうかを確認
	if (!_strnicmp(diskname, filename, MAX_PATH))
	{
		ref++;
		return true;
	}
	return false;
/*
	const char *szDiskName = diskname;
	const char *szFileName = filename;

	for( int i = 0; i < MAX_PATH && *szDiskName != 0 && *szFileName != 0;
			i++ )
	{
		if( toupper( *szDiskName ) != toupper( *szFileName ) )
		{
			return false;
		}
		szDiskName++;
