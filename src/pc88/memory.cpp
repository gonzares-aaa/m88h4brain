...
//	Main 側メモリ(含ALU)の実装
// ----------------------------------------------------------------------------
//	$Id: memory.cpp,v 1.36 1999/12/28 10:34:08 cisc Exp $
...
Memory::Memory(const ID& id)
  :	Device(id), rom(0), ram(0), eram(0), tvram(0), bus(0), 
	dicrom(0), cdbios(0), n80rom(0), mm(0), mid(-1)
{
	txtwnd = 0;
	erambanks = 0;
	neweram = 4;
...
void Memory::UpdateN80R()
{
	...
	if (((porte2 & 0x01) || (port31 & 0x02)) && (porte3 < erambanks))
		read = &eram[porte3 * 0x8000];
	else
		read = n80rom;
...
void Memory::Update60R()
{
	...
	{
		r60 = read;
//		bus->SetReadMemorys2(0x6000, 0x2000, read, this);
...
void Memory::Update00W()
{
	...
	{
		w00 = write;
//		bus->SetWriteMemorys2(0, 0x8000, write, this);
...
void Memory::Update80()
{
	if (port31 != 0)
	{
//		bus->SetMemorys2(0x8000, 0x400, ram + 0x8000, this);
	...
	else
	{
		if (txtwnd <= 0xfc00)
		{
//			bus->SetMemorys2(0x8000, 0x400, ram + txtwnd, this);
			mm->AllocR(mid, 0x8000, 0x400, ram + txtwnd);
			mm->AllocW(mid, 0x8000, 0x400, ram + txtwnd);
		}
		else
		{
//			bus->SetFuncs2(0x8000, 0x400, this, RdWindow, WrWindow);
...
void Memory::SelectJisyo()
{
	if (seldic)
	{
		...
		{
			rc0 = mem;
//			bus->SetReadMemorys2(0xc000, 0x4000, rc0, this);
...
void Memory::UpdateC0()
{
	...
	if (selgvram)
	{
		selgvram = false;
		rc0 = ram + 0xc000;
//		bus->SetMemorys2(0xc000, 0x3000, ram+0xc000, this);
	...
	{
		rc0 = ram + 0xc000;
//		bus->SetReadMemorys2(0xc000, 0x3000, ram+0xc000, this);
		mm->AllocR(mid, 0xc000, 0x3000, ram + 0xc000);
	}
}
...
//		bus->SetMemorys2(0xf000, 0x1000, mem, this);
...
void Memory::UpdateN80G()
{
	...
	{
		rc0 = 0;
//		bus->SetFuncs2(0x8000, 0x4000, this, RdGVRAM0, WrGVRAM0);
		mm->AllocR(mid, 0x8000, 0x4000, RdGVRAM0);
		mm->AllocW(mid, 0x8000, 0x4000, WrGVRAM0);
		selgvram = true;
		return;
	}
	// Normal RAM ? 
	...
	{
		selgvram = false;
//		bus->SetMemorys2(0x8000, 0x4000, ram+0x8000, this);
...
void Memory::SelectGVRAM()
{
	...
	mm->AllocR(mid, 0xc000, 0x4000, rf[port5x & 3]);

	static const MemoryManager::WrFunc wf[3] =
	{
		WrGVRAM0, WrGVRAM1, WrGVRAM2
	};
	mm->AllocW(mid, 0xc000, 0x4000, wf[port5x & 3]);
...
//	bus->SetFuncs2(0xc000, 0x4000, this, RdALU, funcs[(port35 >> 4) & 3]);
	mm->AllocR(mid, 0xc000, 0x4000, RdALU);
	mm->AllocW(mid, 0xc000, 0x4000, funcs[(port35 >> 4) & 3]);
