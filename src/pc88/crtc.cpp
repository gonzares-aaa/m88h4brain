...
// ---------------------------------------------------------------------------
//	$Id: crtc.cpp,v 1.25 2000/01/06 07:58:50 cisc Exp $
...
{
	const uint8* src = fontrom;
	uint8* dest = font;
	uint8* destw = font + 0x8000;

	for (uint i=0; i<256*8; i++)
	{
		uint8 d = *src++;
...
// ---------------------------------------------------------------------------
//	VSYNC を外部から供給するモードでの更新
//
void CRTC::SyncUpdate()
{
	if (syncmode)
	{
	...
	if (!(mode & skipline) || !(column & 1))
	{
		if (status & 0x10)
		{
			...
			else
			{
				...
				if ((mode & control) && (dest[linesize-2] & 0x7f) == 0x60)
				{
					// 特殊制御文字
...
{
	for (uint y=400; y>0; y--)
	{
		packed* d = REINTERPRET_CAST(packed*, dest);

		for (uint x=640/sizeof(packed)/4; x>0; x--)
		{
			d[0] = (d[0] & pat_mask) | TEXT_RESP;
			d[1] = (d[1] & pat_mask) | TEXT_RESP;
			d[2] = (d[2] & pat_mask) | TEXT_RESP;
			d[3] = (d[3] & pat_mask) | TEXT_RESP;
			d+=4;
		}
		dest += bpl;
	}
...
void CRTC::ExpandImage(uint8* image, Draw::Region& region)
{
	...
	int yy = Min(400 / linesperchar, height) - 1;
...
//	LOG0("\n");
	region.Update(linesperchar * top, 
...
void CRTC::ExpandAttributes(uint8* dest, const uint8* src, uint y)
{
	...
	if (cursor_y == y && attr_cursor)
	{
		...
		for (uint i=0; i<width; i++)
		{
			...
			dest[i] = !cx-- ? attr ^ attr_cursor : attr;
		}
	}
	else
	{
...
//	同期ソースを指定
//
void CRTC::SetSyncMode(bool syncoutside)
{
	if (syncmode != syncoutside)
	{
