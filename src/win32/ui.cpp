...
// ---------------------------------------------------------------------------
//	$Id: ui.cpp,v 1.45 2000/01/10 08:25:34 cisc Exp $
...
: hinst(hinstance), extlist(0), uiindex(0xc0)
{
...
bool WinUI::InitM88(const char* cmdline)
{
	...
	if (!keyif.Init(hwnd))
		return false;
	if (!core.Init(this, hwnd, &draw, diskmgr, &keyif, &winconfig))
		return false;
	
	//	debug 用クラス初期化
	opnmon.Init(core.GetOPN1());
	memmon.Init(&core);
	codemon.Init(&core);
...
LRESULT WinUI::WinProc(HWND hwnd, UINT umsg, WPARAM wp, LPARAM lp)
{
	...
	{
		CriticalSection::Lock lock(csextlist);
		for (ExtNode* n = extlist; extlist; extlist=extlist->next)
			extlist->ext->WinProc(hwnd, umsg, wp, lp);
	}
...
uint WinUI::WmCommand(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	...
	case IDM_DUMPCPU1:
		core.EnableCPUDump(0, !core.GetCPU1()->GetDumpState());
		break;

	case IDM_DUMPCPU2:
		core.EnableCPUDump(1, !core.GetCPU2()->GetDumpState());
		break;
...
uint WinUI::WmInitMenu(HWND hwnd, WPARAM wp, LPARAM lp)
{
	...
	CheckMenuItem(hmenu, IDM_WATCHREGISTER, (config.dipsw != 1 && (config.flags & Config::watchregister)) ? MF_CHECKED : MF_UNCHECKED);
...
void WinUI::GetSnapshotName(char* name, int n)
{
	...
	if (diskmgr->GetNumDisks(0))
		GetFileNameTitle(title, diskinfo[0].filename);
	else
		strcpy(title, "snapshot");
	
	if (n >= 0)
		wsprintf(name, "%s_%d.s88", title, n);
	else
		wsprintf(name, "%s_?.s88", title, n);
}
...
//
//
bool IFCALL WinUI::HookExtention(IWinUIExtention* e)
{
	CriticalSection::Lock lock(csextlist);
	if (uiindex < 0x100)
	{
		if (e->Connect(hwnd, uiindex))
		{
			ExtNode* n = new ExtNode;
			if (!n)
			{
				e->Disconnect();
				return false;
...
//
//
bool IFCALL WinUI::UnhookExtention(IWinUIExtention* e)
{
	CriticalSection::Lock lock(csextlist);

	for (ExtNode** r = &extlist; *r; r=&((*r)->next))
	{
		if ((*r)->ext == e)
		{
			ExtNode* d = *r;
