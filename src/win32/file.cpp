...
char FileIO::m_szCurrentDirectory[MAX_PATH];
...
//	strcpy( path, "\\My Documents\\devtest\\" );
...
//	LPWSTR szWFileName = MultiToWide( filename );
	LPWSTR szWFileName = MultiToWide( path );
	hfile = CreateFile(szWFileName, access, share, 0, creation, 0, 0);
	free( szWFileName );
...
bool FileIO::Reopen(uint flg)
{
	...
	LPWSTR szWPath = MultiToWide( path );
	hfile = CreateFile(szWPath, access, share, 0, creation, 0, 0);
	free( szWPath );
...
void FileIO::SetCurrentDirectory( const char *szDirectory )
{
	strcpy( m_szCurrentDirectory, szDirectory );
}


/*---------------------------------------------------------------------------
		フルパス名を取得する（超手抜き版）
		出力先のバッファサイズは MAX_PATH とする。
---------------------------------------------------------------------------*/

void FileIO::GetFullPath( char *szDst, const char *szFileName )
{
	if( *szFileName == '\\' )
	{
		//
		//	元のファイル名の先頭が \ だったら元々フルパスと判断する(ぉぃ
		//
	...
	//
	//	そうでなければカレントディレクトリと結合する。
	//	相対パスとかは考えてないです....(^^;
	//
	int iCurDirLen = strlen( m_szCurrentDirectory ) + 1;
	int iFileNameLen = strlen( szFileName );
	int iTotalLen = iCurDirLen + iFileNameLen;

	if( iTotalLen > MAX_PATH )
	{
		iFileNameLen = MAX_PATH - iCurDirLen;
	}

	strcpy( szDst, m_szCurrentDirectory );
	strcat( szDst, "\\" );
	strncpy( szDst + iCurDirLen, szFileName, iFileNameLen );
	*( szDst + iCurDirLen + iFileNameLen ) = 0;
}
