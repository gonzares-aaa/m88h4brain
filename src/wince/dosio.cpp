...
BRESULT file_close(FILEH handle) {
...
static BRESULT cnvdatetime(FILETIME *file,
...
BRESULT file_getdatetime(FILEH handle, DOSDATE *dosdate, DOSTIME *dostime) {
...
BRESULT file_delete_c(const OEMCHAR *path) {

	file_cpyname(curfilep, path, NELEMENTS(curpath) - (curfilep - curpath));
	return(file_delete(curpath));
}

SINT16 file_attr_c(const OEMCHAR *path) {
